...
**复合意图模式下的确认策略**：
- 复合意图模式下（用户说"全套"、"从头到尾"、"检查并补全"等），各子技能的确认步骤默认自动通过，不逐一询问
  - req-wireframe 的占位符清单确认 → 自动跳过
  - req-testcase 的模块列表确认 → 自动跳过
  - req-flowdoc 的流程列表确认 → 自动跳过
- 除非用户在触发时明确说"每步让我确认"，才恢复逐项确认
...
当执行过程中遇到以下情况，按对应策略处理而不是报错停止：

| 情况 | 降级行为 |
|------|---------|
| `Glob("**/prd-template.md")` 等 shared 文件搜索返回 0 结果 | 使用 SKILL.md 中内置的格式规则继续，并提示用户"未找到模板文件，使用内置规则" |
| `reference/business-rules.md` 内容为空模板（只有 HTML 注释） | 跳过项目特定规则检查，只执行通用规则；在口头汇总中注明"未配置项目业务规则" |
| `output/review-issues.md` 不存在但执行 req-complete | req-complete 自行执行一轮快速自查（不中断），在 changelog 中注明"无前置审查结果" |
| 截图工具不可用（req-wireframe） | 保留 `{低保真原型图}` 占位符不替换，在 changelog 记录"截图待手动完成：open output/wireframes/{文件名}.html" |
| 输入文档无任何 `#` 标题（纯文本） | 标记为 C 类输入，整体提取后用 prd-template.md 重建结构 |
| 模型生成内容较长导致中断 | 优先保证 `#### 3.2.x` 节完整，其他节可标 TBD；文档末尾加"[生成未完成，以下内容待补充]"提示 |
| 用户回答了"待定"/"TBD" | 标注 `[TBD-ISS-NNN]` 继续下一条，不追问，不强制解决 |

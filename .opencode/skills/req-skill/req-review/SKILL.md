...
1. **通用规则**：读取 `req-review/generic-rules.md`，这是内置的、不依赖项目的通用审查维度
2. **项目规则**：读取 `reference/business-rules.md`，这是项目团队自定义的业务规则

如果 `reference/business-rules.md` 为空或不存在，只使用通用规则，不报错。
...
按 `shared/issue-schema.md` 中定义的结构输出缺陷清单到 `output/review-issues.md`。
